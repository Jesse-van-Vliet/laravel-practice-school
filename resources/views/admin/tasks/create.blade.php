...
@section('content')
    <div class="card nt-6">
        ...
        <div class="card-body grid grid-cols-1 gap-6 lg:grid-cols-1">
            <div class="p-4">
                <form id="form" class="shadow-md rouded-lg px-8 pt-6 pb-8 mb-4" action="{{ route('tasks.store')  }}" method="POST">
                    @csrf
                    <label class="block tex-sn">
                        <span class="text-gray-700">
                            task
                        </span>
                        <input class="bg-gray-200 block rounded w-full p-2 nt-2 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple form-input" name="task" value="{{old('task')}}" type="text" required>
                        <span class="text-gray-700">
                            User
                        </span>
                        ...
                        <select class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" name="project_id" id="project_id">
                            @foreach($project as $projects)
                                <option value="{{$projects->id}}" @selected($projects->id = old('user_id'))> {{ $projects->name }}</option>
                            @endforeach
                        ...
                        <select class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" name="activity_id" id="activity_id">
                            @foreach($activity as $activities)
                                <option value="{{$activities->id}}" @selected($activities->id = old('activity_id'))> {{ $activities->name }}</option>
                            @endforeach
                        ...
                        <input class="border-2 m-4" type="date" id="start" name="begindate" value="{{Carbon\Carbon::now()->format('Y-m-d')}}" min="{{Carbon\Carbon::now()->format('Y-m-d')}}" />
                        ...
                        <input class="border-2 m-4" type="date" id="start" name="enddate" value="{{Carbon\Carbon::now()->format('Y-m-d')}}" min="{{Carbon\Carbon::now()->format('Y-m-d')}}" />
